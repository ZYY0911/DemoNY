        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    YGXX demoBean = (YGXX) objList.get(j);
                    List<String> list = new ArrayList<>();
                    list.add(demoBean.getName());
                    list.add(demoBean.getSex());
                    list.add(demoBean.getBirth());
                    list.add(demoBean.getTel());
                    list.add(demoBean.getScx());
                    list.add(demoBean.getZw());
                ...
                Toast.makeText(c, "导出Excel失败"+e.getMessage(), Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            } finally {
                if (writebook != null) {
                    try {
